...
st.image('daily_shabdakhool.png')
    ...
    with placeholder.container():
        ...
        if onemore: # If victory has not been achieved
            ...
            with col1: # शब्दखूुळ doesn't show properly everywhere
                st.write("दवंडी पिटा")
                toShare = "दैनिक शब्दखूुळ तिनाक्षरी\n#%d %s/∞\n\n%s\n[%s](%s)" % (st.session_state['nthword'],get_mdigits(len(st.session_state['mylist'])-1),modalstr,purl,purl)
                st.code(toShare)
            myc2 = ''
...
def main():
    ...
    secret_wordfile = "words_%s.dat" % pmode 
    ...
    if 'secret' not in st.session_state:
        ...
        if nthword < 58:
            nthword += 365
        nthword -= 57   # sets 1 for 27 Feb == marathi divas
...
if __name__ == "__main__":
    main() # wlen is the global at the top